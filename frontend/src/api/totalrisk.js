// frontend/src/api/totalrisk.js
import axiosInstance from './axiosconfig';

const fetchLocalData = async () => {
  try {
    const response = await axiosInstance.get('totalrisk/total-risk/');
...
const fetchHanaData = async () => {
  try {
    const response = await axiosInstance.get('totalrisk/fetch-hana-data/');
    return response.data; // Başarılı yanıt döndür
  } catch (error) {
    // Hata durumunda error objesini döndür
...
const getLastUpdatedTime = async () => {
  try {
    const response = await axiosInstance.get('totalrisk/last-updated/');
...
const exportTotalRiskToXLSX = async () => {
  try {
    const response = await axiosInstance.get('totalrisk/export-xlsx/', { responseType: 'blob' });
