// frontend/src/api/tunainssupplierpayment.js
import axiosInstance from './axiosconfig';

const getLocalDbSupplierPayments = async () => {
  try {
    const response = await axiosInstance.get('tunainssupplierpayment/local_db_closing_invoice');
...
const fetchHanaDbCombinedService = async () => {
  try {
    const response = await axiosInstance.get('tunainssupplierpayment/fetch_hana_db_combined_service/');
...
const getLastUpdatedTime = async () => {
  try {
    const response = await axiosInstance.get('tunainssupplierpayment/last_updated_supplier_payment/');
...
const getTaskStatus = async (taskId) => {
  try {
    const response = await axiosInstance.get(`tunainssupplierpayment/task_status/${taskId}/`);
    return response.data;
  } catch (error) {
...
const fetchHanaDb = async () => {
  try {
    const response = await axiosInstance.get('tunainssupplierpayment/fetch_hana_db/');
    // Başarılı yanıt
...
const createWebSocketConnection = (onMessage) => {
  ...
  const wsUrl = `${wsProtocol}//${host}/ws/tunainssupplierpayment/`;
