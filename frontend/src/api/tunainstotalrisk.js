// frontend/src/api/tunainstotalrisk.js
import axiosInstance from './axiosconfig';

const fetchLocalData = async () => {
  try {
    const response = await axiosInstance.get('tunainstotalrisk/total-risk/');
...
const fetchHanaData = async () => {
  try {
    const response = await axiosInstance.get('tunainstotalrisk/fetch-hana-data/');
    return response.data; // Başarılı yanıt döndür
  } catch (error) {
    // Hata durumunda error objesini döndür
...
const getLastUpdatedTime = async () => {
  try {
    const response = await axiosInstance.get('tunainstotalrisk/last-updated/');
...
const exportTotalRiskToXLSX = async () => {
  try {
    const response = await axiosInstance.get('tunainstotalrisk/export-xlsx/', { responseType: 'blob' });
