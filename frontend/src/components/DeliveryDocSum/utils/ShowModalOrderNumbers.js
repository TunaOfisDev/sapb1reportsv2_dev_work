// frontend/src/components/DeliveryDocSum/utils/ShowModalOrderNumbers.js
...
const ShowModalOrderNumbers = ({ modalIsOpen, closeModal, selectedOrders, cariKod, cariAdi }) => {
    return (
        ...
        >
            ...
            {selectedOrders ? selectedOrders.split(',').map((number, index) => <p key={index}>{number.trim()}</p>) : <p>No numbers available.</p>}
