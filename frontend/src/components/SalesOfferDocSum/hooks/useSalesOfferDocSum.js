// frontend/src/components/SalesOfferDocSum/hooks/useSalesOfferDocSum.js
import { useState, useEffect, useCallback } from 'react';
import salesOfferDocSumApi from '../../../api/salesofferdocsum';

const useSalesOfferDocSum = () => {
  const [salesOfferDetails, setSalesOfferDetails] = useState([]);
  ...
  const fetchSalesOfferDetails = useCallback(async () => {
    setLoading(true);
    try {
      const data = await salesOfferDocSumApi.fetchSalesOfferDetails();
      setSalesOfferDetails(data);
    } catch (err) {
      setError(err);
    } finally {
      setLoading(false);
  ...
  const fetchHanaData = useCallback(async () => {
    setLoading(true);
    try {
      const data = await salesOfferDocSumApi.fetchHanaData();
      setHanaData(data);
  ...
  const fetchSalesOfferDetailByBelgeNo = useCallback(async (belgeNo) => {
    setLoading(true);
    try {
      const data = await salesOfferDocSumApi.fetchSalesOfferDetailByBelgeNo(belgeNo);
      return data;
    } catch (err) {
      setError(err);
      throw err;
  ...
  const fetchDocumentSummaries = useCallback(async () => {
    setLoading(true);
    try {
      const data = await salesOfferDocSumApi.fetchDocumentSummaries();
  ...
  const fetchFilteredDateDocSum = useCallback(async (filters) => {
    setLoading(true);
    try {
      const data = await salesOfferDocSumApi.fetchFilteredDateDocSum(filters);
...
export default useSalesOfferDocSum;