// frontend/src/components/TunaInsTotalRisk/containers/TotalRiskTable.js
...
const subtotals = useDynamicSubtotals(rows, 'muhatap_kod', ['bakiye', 'acik_teslimat', 'acik_siparis','avans_bakiye', 'toplam_risk']);
  ...
  <>
    {/* Pagination */}
    <div className="totalrisk-pagination__wrapper">
      <Pagination
    ...
    <table {...getTableProps()} className="total-risk-table">
      <thead className="total-risk-table__head">
        {headerGroups.map((headerGroup, hgIdx) => (
          <tr
            ...
            className="total-risk-table__row"
          >
            {headerGroup.headers.map((column) => (
              <th
                key={column.id}
                {...column.getHeaderProps()}
                className="total-risk-table__cell"
              >
                ...
                {column.canFilter && (
                  <div className="totalrisk-columnfilter-container">
      ...
      <tbody {...getTableBodyProps()} className="total-risk-table__body">
        {page.map((row) => {
          prepareRow(row);
          return (
            <tr key={row.id} {...row.getRowProps()} className="total-risk-table__row">
              {row.cells.map((cell) => (
                <td
                  key={cell.column.id}
                  {...cell.getCellProps()}
